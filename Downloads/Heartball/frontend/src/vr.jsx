...
const useGameStore = create((set) => ({
  ...
  sackOccurred: false,
  // Player positions, managed in the store to avoid ref-related timing issues
  qbPosition: new Vector3(0, 1.5, -5),
  defenderPosition: new Vector3(10, 1.5, -5),
...
// Dedicated component to run the game loop logic
// It now directly reads and updates state from the store
const GameLoop = () => {
  const { playerRole, vrMode, sackOccurred, setVrMode, setSackOccurred, setMessage, qbPosition, defenderPosition } = useGameStore();

  useFrame(({ camera }) => {
    // If a sack hasn't occurred, move the defender towards the QB
    if (!sackOccurred) {
      // Calculate the distance between QB and Defender
      const distance = qbPosition.distanceTo(defenderPosition);
      ...
      if (distance < 2.5) {
        ...
        const newDefenderPosition = defenderPosition.clone().lerp(qbPosition, 0.01);
        useGameStore.setState({ defenderPosition: newDefenderPosition });
      }
    }

    // Camera control based on vrMode
    if (vrMode === 'qb-view') {
      camera.position.lerp(qbPosition.clone().add(new Vector3(-2, 1.5, -2)), 0.1);
      camera.lookAt(defenderPosition);
      camera.fov = 90; // Adjust FOV for an immersive feel
      camera.updateProjectionMatrix();
    } else if (vrMode === 'defender-view') {
      camera.position.lerp(defenderPosition.clone().add(new Vector3(2, 1.5, 2)), 0.1);
      camera.lookAt(qbPosition);
      camera.fov = 90; // Adjust FOV for an immersive feel
      camera.updateProjectionMatrix();
    } else {
...
const GameScene = () => {
  const { vrMode, qbPosition, defenderPosition } = useGameStore();
  ...
  return (
    <>
      ...
      <group position={defenderPosition}>
      ...
      {/* The separate GameLoop component handles all animation logic */}
      <GameLoop />
